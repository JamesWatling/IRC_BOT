...
class Cinch::GithubCommits
  ...
  post "/commit" do
    params[:payload] = request.body.read#.raw_post

    #bot.channels.each{|c| c.send("Someone commited something to mock just now")}
    ...
    post "/proto_commit" do
    #bot.channels.each{|c| c.send("Someone commited something to prototype just now")}
    params[:payload] = request.body.read#.raw_post
